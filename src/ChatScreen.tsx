...
import { Alert, Linking, Platform, StyleSheet, Text, View, KeyboardAvoidingView } from 'react-native'
import {
  GiftedChat,
  IMessage,
  Send,
...
// export const sendMessage = async (channelId:any, message:any) => {
//   await firestore().collection('chat').doc(channelId).collection('messages').add(message);
// };

// export const getMessages = async (channelId:any) => {
//   const snapshot = await firestore().collection('chat').doc(channelId).collection('messages').orderBy('createdAt').get();
...
const ChatScreen = ({ channelId}:any) => {
  ...
  //   // 실시간 메시지 업데이트
  //   const unsubscribe = firestore().collection('chat').doc(channelId).collection('messages')
  //     .orderBy('createdAt')
  //     .onSnapshot(snapshot => {
  ...
  // const onSend = useCallback(
  //   async (messages:any) => {
  //     const sentMessages = [{ ...messages[0], sent: true, received: true, createdAt: new Date() }];
  
  //     // Firebase에 메시지 전송
  //     await sendMessage(channelId, sentMessages[0]);
  
  //     // GiftedChat 업데이트
  //     const newMessages = GiftedChat.append(messages, sentMessages, false);
  //     dispatch({ type: ActionKind.SEND_MESSAGE, payload: newMessages });
  //   },
  //   [channelId, dispatch]
  // );
